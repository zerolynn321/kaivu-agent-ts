...
Keep post-baseline source inspection, requirement-to-code traceability, planned method and integration changes, post-change baseline regression, formal experiment planning, per-branch dry runs, batch script generation, README formal-run instructions, direct codebase verification, and the `ready_for_formal_run` gate in `repo-experiment-prepare`. Treat it as AgentExperimentPrepare, not as part of AgentInit or AgentFix. The primary deliverable is the working experiment codebase; artifacts only record evidence and must not replace direct inspection of code, commands, and outputs.

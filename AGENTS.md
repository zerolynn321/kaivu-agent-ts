...
- `repo-experiment-prepare`: after baseline validation, act as AgentExperimentPrepare in `optimization` mode for a specific paper/repository or `requirement_validation` mode for an open-ended research need; make the current server workspace ready for formal experiments by building one coherent locally complete codebase with an explicit architecture, root formal entrypoint, real programmatic component interfaces, automatic end-to-end artifact flow, and unified experiment controls; preserve the baseline, verify the complete bounded workflow, freeze and dry-run the experiment matrix, document exact commands in README, and produce a consolidated human-readable `experiment_readiness_report.md`. Do not require Git initialization, commits, tracked files, author identity, or clean-clone reconstruction.
